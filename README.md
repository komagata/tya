...
Tya v0.8 is a compile-to-C language. The reference implementation contains a Go
lexer, parser, AST, checker, C emitter, C runtime, CLI, examples, and v0.8
specification tests.
...
For v0.8.0, download the release source and build the `tya` command locally.
This currently requires Go because the v0.8 reference implementation is written
in Go.

```sh
curl -L https://github.com/komagata/tya/archive/refs/tags/v0.8.0.tar.gz | tar xz
cd tya-0.8.0
...
The current released implementation, Tya v0.8, includes:
...
- minimal classes, constructor calls, `init`, public instance fields,
  ...
  single inheritance, class-level inheritance, class introspection, method
  overrides, class-method `self`, and `super(args...)`

Tya v0.8 does not include interfaces, multiple inheritance, visibility
