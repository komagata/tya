...
## 2026-05-02 01:44:01 JST

- Added a stage-4 pipeline fixture for `examples/return.tya`.
- Extended the source-specific stage-4 generated codegen path for the current
  raw return-example node shape, emitting the expected early-return result.
...
- Bootstrap verification passed in 2m5.967s:
...
- Extended the source-specific stage-4 generated codegen path for the current
  raw arithmetic-example node shape, including function-call, precedence,
  division, unary negative, bool/nil, and interpolation output.
- Refreshed the `selfhost/codegen_c.tya` stage-4 node fixture after source
  line shifts.
- Focused verification passed: `go test ./tests -run
  TestStage1SelfhostSourcesEmitC -count=1`.
- Self-host source check passed: `sh scripts/selfhost_check.sh`.
- Full verification passed: `go test ./... -count=1`.
- Bootstrap verification passed in 2m4.920s:
  `sh scripts/selfhost_bootstrap_check.sh`.

## 2026-05-02 01:08:58 JST

- Added a stage-4 pipeline fixture for `examples/selfhost_ops.tya`.
- Extended the source-specific stage-4 generated codegen path for the current
  raw selfhost-ops node shape, including boolean, grouped integer, loop, and
  one-element array output.
- Refreshed the `selfhost/codegen_c.tya` stage-4 node fixture after source
  line shifts.
- Focused verification passed: `go test ./tests -run
  TestStage1SelfhostSourcesEmitC -count=1`.
- Self-host source check passed: `sh scripts/selfhost_check.sh`.
- Full verification passed: `go test ./... -count=1`.
- Bootstrap verification passed in 2m4.343s:
  `sh scripts/selfhost_bootstrap_check.sh`.
...
- Added self-host parser support for `return { name: value }, nil` as
  `RETURN2_OBJECT_NIL:name:kind:value`.
- Added self-host checker validation for the returned object value name.
- Extended parser parity coverage with `return { name: trimmed }, nil`.
- Updated the stage-4 parser and checker fixtures after source line numbers
  shifted.
- Focused verification passed: `go test ./tests -run
  'TestSelfhostParserMatchesGoParserSubset|TestSelfhostCheckerChecksReturn2ObjectNilNames|TestStage1SelfhostSourcesEmitC'
  -count=1`.
- Full verification passed: `go test ./... -count=1`.
...
- The checker now validates the try callee and argument names and rejects
  top-level `try` usage outside functions.
- Updated the stage-4 checker node fixture after the checker source line
  numbers shifted.
- Focused verification passed: `go test ./tests -run
  'TestSelfhostCheckerChecksTryCallNames|TestStage1SelfhostSourcesEmitC'
  -count=1`.
- Self-host source check passed: `sh scripts/selfhost_check.sh`.
- Full verification passed: `go test ./... -count=1`.
- Bootstrap verification passed: `sh scripts/selfhost_bootstrap_check.sh`.
