...
- [-] Adopt Canonical Syntax (multi-version landing per `docs/CANONICAL_SYNTAX.md`)
  ...
  - [x] **v0.38** Step 6: Canonical Syntax landing. `tya fmt` defaults to the AST-driven serializer; selfhost compiler extended for canonical wrap forms; `examples/` / `stdlib/` / `selfhost/` normalized; self-host fixed point preserved.
  - Deferred to v0.38.x / v0.39:
    ...
    - [ ] Remove the `--text` opt-out once Unparse covers every AST shape v0.1 supports.
    - [ ] Promote `tya fmt` to `tya format` as the only canonical spelling.
