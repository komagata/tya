...
- expression statement、return、assignment、class member の長い array
  literal は、rendered line が column limit を超える場合に
  formatter-defined block form に整形される。長い call argument と nested
  collection value は、grammar が受け付ける場所では multi-line array/dict
  block form を使える
- triple-quoted string literal は triple-quoted かつ multi-line のまま保つ。
  `"""...""".trim()` のように method call の receiver として使う場合も同じである
...
nil 合体演算子 `??` は、左オペランドが `nil` でなければそれを評価結果として返す。
左オペランドが `nil` の場合だけ右オペランドを評価して返す。フォールスルーするのは
`nil` だけであり、`false`、`0`、`0.0`、`""`、空配列、空辞書は保持される。

```tya
name = maybe_name ?? "anonymous"
```

算術演算は、文書化されたプリミティブメソッドまたは演算子ケースが別のことを述べる場合を除き、数値を要求する。`+` は文字列同士、またはバイト値同士だけを連結し、混在オペランドを暗黙に文字列化しない。文字列補間は `Stringable` surface で埋め込まれた値を整形する。`nil` 算術は不正である。
...
nil 合体代入は `??=` を使い、`=` と同じ単一代入ターゲットに対して使える。
ターゲットの現在値を読み、その値が `nil` の場合だけ右辺を評価して代入する。
現在値が `nil` でなければ、右辺は評価されず、代入も行われない。
代入を発生させるのは `nil` だけであり、Ruby の `||=` と違って `false`、`0`、
`0.0`、`""`、空配列、空辞書は既存値を保つ。
